...
OUTPUT_DIR = RESULT_ROOT / "robotics"

FP32_RESULT = RESULT_ROOT / "fp32" / "robotics-fp32-seed-42.json"

FP32_TARGETS = RESULT_ROOT / "fp32-targets.json"

SPRINT2_SELECTION = (
    Path("results") / "compression" / "robotics-compression-selection.json"
)
...
def _validate_sprint2_selection() -> None:
    ...
    if payload["domain"] != Domain.ROBOTICS.value:
...
def _load_fp32_target() -> TargetDefinition:
    ...
    matches = [
        ...
        if (item["domain"] == Domain.ROBOTICS.value and item["seed"] == SEED)
    ]

    if len(matches) != 1:
        raise RuntimeError("expected exactly one robotics " "seed-42 FP32 target")
    ...
    if abs(expected_target - target_loss) > 1e-12:
        raise RuntimeError("stored FP32 target violates " "the frozen 5% rule")
...
def _train_candidate(
    *,
    ...
    best_epoch = next(
        ...
        if (item.validation_loss == completed_best)
    )
...
def main() -> None:
    ...
    payload = {
        "experiment_id": ("robotics-training-efficiency-seed-42"),
        "domain": (Domain.ROBOTICS.value),
        "seed": SEED,
        "exploratory": True,
        "protocol": {
        ...
        "configurations": {
            ...
            "svd": {
                "method": ("trainable_svd"),
                "rank_fraction": (SVD_RANK_FRACTION),
                "selection_source": ("Sprint 2 robotics selection"),
            },
            "tt_mps": {
                "method": ("trainable_tt_mps"),
                "max_rank": (TT_MAX_RANK),
        ...
        "limitations": [
            (
                "Single-seed exploratory robotics result; "
    ...
    output_path = OUTPUT_DIR / "robotics-training-efficiency-seed-42.json"
    ...
    print()
    print("============================================")
    print(" Q-VLA FORGE — ROBOTICS TRAINING EFFICIENCY")
    print("============================================")
