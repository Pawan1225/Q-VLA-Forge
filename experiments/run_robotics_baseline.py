...
def main() -> None:
    result = run_robotics_baseline(
        ...
        epochs=20,
        batch_size=32,
        output_path=Path("results/" "robotics-baseline-seed-42.json"),
    )

    print()
    print("===== Q-VLA Forge " "Robotics Baseline =====")
    ...
    print(f"  Delta-X MAE: " f"{result.test_metrics.delta_x_mae:.6f}")

    print(f"  Delta-Y MAE: " f"{result.test_metrics.delta_y_mae:.6f}")

    print(f"  Gripper MAE: " f"{result.test_metrics.gripper_mae:.6f}")
    ...
    print("  results/" "robotics-baseline-seed-42.json")
