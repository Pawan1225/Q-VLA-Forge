"""Select seed-42 robotics compression candidates for validation."""
...
OUTPUT_PATH = COMPRESSION_DIR / "robotics-compression-selection.json"

CSV_PATH = COMPRESSION_DIR / "robotics-compression-comparison.csv"
...
def _result_files() -> list[Path]:
    """Return all seven seed-42 robotics compression evidence files."""
    files = [
        COMPRESSION_DIR / "robotics-int8-seed-42.json",
    ]

    files.extend(sorted(COMPRESSION_DIR.glob("robotics-svd-*-seed-42.json")))

    files.extend(sorted(COMPRESSION_DIR.glob("robotics-tt-rank-*-seed-42.json")))
...
def _write_csv(
    ...
    """Write compact robotics comparison evidence."""
    lines = [
        (
...
def main() -> None:
    """Normalize and select robotics compression candidates."""
    files = _result_files()

    if len(files) != 7:
        raise RuntimeError("expected exactly seven robotics compression candidates")

    missing = [path for path in files if not path.exists()]

    if missing:
        raise FileNotFoundError(
            "missing robotics compression evidence: "
    ...
    print()
    print("===== Q-VLA Forge " "Robotics Compression Selection =====")
