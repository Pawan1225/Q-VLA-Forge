"""Hybrid QML PPO training using the frozen Sprint 4 PPO protocol."""
...
class HybridPPOTrainingResult:
    """Evidence returned by one hybrid QML PPO run."""
...
def train_hybrid_ppo(
    *,
    ...
    """Train the frozen hybrid QML actor under classical PPO."""
    ...
    if not isinstance(
        ...
        raise TypeError("hybrid PPO requires a Box observation space")

    if not isinstance(
        env.action_space,
        gym.spaces.Box,
    ):
        raise TypeError("hybrid PPO requires a Box action space")

    if env.observation_space.shape is None:
        raise TypeError("hybrid PPO requires a shaped observation space")
    ...
    hybrid_config = HybridQMLPolicyConfig(pqc_seed=seed)
    ...
    return HybridPPOTrainingResult(
        seed=seed,
        domain=domain,
